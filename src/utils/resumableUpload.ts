...
export async function resumableUploadVideo(
  ...
  try {
    ...
    // 上传每个分片（并发上传）
    const uploadedChunks: Blob[] = new Array(totalChunks);
    
    // 先填充已上传的分片
    for (let i = 0; i < totalChunks; i++) {
      if (uploadState.uploadedChunks.includes(i)) {
        uploadedChunks[i] = chunks[i];
      }
    }
    
    // 获取待上传的分片索引
    const pendingChunkIndexes = Array.from({ length: totalChunks }, (_, i) => i)
      .filter(i => !uploadState.uploadedChunks.includes(i));
    
    console.log(`📋 待上传分片: ${pendingChunkIndexes.length} 个`);
    ...
    const uploadChunk = async (chunkIndex: number): Promise<void> => {
      ...
      const { data: chunkData, error: chunkError } = await supabase.storage
        .from('videos')
        .upload(chunkFileName, chunks[chunkIndex], {
          cacheControl: '3600',
          upsert: true // 允许覆盖，支持重试
        });
      
      if (chunkError) {
        console.error(`❌ 分片 ${chunkIndex + 1} 上传失败:`, chunkError);
        throw new Error(`分片 ${chunkIndex + 1} 上传失败: ${chunkError.message}`);
      }
      ...
      uploadState.uploadedChunks.sort((a, b) => a - b); // 保持顺序
      saveUploadState(fileId, uploadState);
      
      uploadedChunks[chunkIndex] = chunks[chunkIndex];
      
      // 更新进度
      const uploadedBytes = uploadState.uploadedChunks.length * CHUNK_SIZE;
      const percentage = Math.min((uploadedBytes / file.size) * 100, 100);
      onProgress?.(uploadedBytes, file.size, percentage);
      onChunkComplete?.(uploadState.uploadedChunks.length, totalChunks);
    };
    ...
    for (let i = 0; i < pendingChunkIndexes.length; i += CONCURRENT_CHUNKS) {
      ...
      console.log(`🔄 并发上传批次: 分片 ${batch.map(idx => idx + 1).join(', ')}`);
