...
/**
 * 自动更新 videos.total_sentences 字段
 * 使用 Node.js 和 Supabase 客户端
 */
    ...
    try {
        ...
        if (data && data.length > 0) {
            ...
            if (hasColumn) {
                ...
                log('total_sentences 字段不存在', 'error');
                log('请先在 Supabase SQL Editor 中执行:', 'warning');
                log('ALTER TABLE videos ADD COLUMN IF NOT EXISTS total_sentences INTEGER DEFAULT 0;', 'warning');
                return false;
            }
        }
...
async function updateData() {
    try {
        ...
        for (let i = 0; i < videos.length; i++) {
            ...
            if (annError) {
                log(`[${i + 1}/${videos.length}] 查询视频 ${video.name} 的标注失败`, 'error');
                errorCount++;
                continue;
            }
            ...
            if (video.total_sentences === totalSentences) {
                skippedCount++;
                process.stdout.write(`\r⏭️  [${i + 1}/${videos.length}] 处理中... (${video.name}: ${totalSentences}句)`);
                continue;
            }
            ...
            const { error: updateError } = await supabase
                .from('videos')
                .update({ total_sentences: totalSentences })
                .eq('id', video.id);
...
async function verifyResults() {
    try {
        ...
        const { data: videos, error } = await supabase
            .from('videos')
            .select('id, name, total_sentences, created_at')
